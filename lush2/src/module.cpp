...
nsbundle_hset(const char *sname, nsbundle_t *mark)
{
  ...
  if (! p)
    {
      p = (nsbundle_sym_s*)malloc(sizeof(struct nsbundle_sym_s));
      p->name = strdup(sname);
      p->left = p->right = 0;
      *pp = p;
    }
...
nsbundle_symmark(nsbundle_t *bundle, nsbundle_t *mark)
{
  ...
  int ns = bundle->di->num_export();
  while (--ns >= 0)
    {
      //const char *sname = NSSymbolDefinitionNameInObjectFileImage(nsimg, ns);
      const char *sname = bundle->di->kth_export(ns);
...
nsbundle_unload(nsbundle_t *bundle)
{
  ...
  if (bundle->di) { delete bundle->di; bundle->di = 0; }
  if (bundle->nsimage)
...
nsbundle_load(const char *fname, nsbundle_t *bundle)
{
  ...
  if ((cmd = (char*)malloc(fnamelen + 256)) && (bundle->name = (char*)malloc(256)))
    {
#if DEBUGNAMES
...
#endif
      sprintf(cmd,
	      "ld -bundle -flat_namespace -macosx_version_min 10.6 -undefined suppress /usr/lib/bundle1.o \"%s\" -o \"%s\"",
	      fname, bundle->name);
      ...
      if (system(cmd) == 0)
	{
	  ...
	  if ((ret == NSObjectFileImageSuccess) &&
	      (bundle->di = get_bundle_info(bundle->name)) &&
...
{
  void *addr = 0;
  char *usname = (char*)malloc(strlen(sname)+2);
  if (usname)
    {
      strcpy(usname, "_");
      strcat(usname, sname);
...
{
   module_t *m = (module_t*)mm_alloc(mt_module);
   m->flags = 0;
   m->initaddr = 0;
...
extern "C" void *dynlink_symbol(module_t *m, const char *sname, int func, int exist)
{
#if DLDBFD
   if (func)
...
extern "C" void clean_dhrecord(dhrecord *);
         ...
         if (CONSP(Car(p))) {
		  ...
		  kdoc = (dhdoc_t*)cfunc->info;
               ...
               if (! newstate)
                  ...
                  dhclassdoc_t *kdata = (dhclassdoc_t*)dynlink_symbol(m, cl->kname, 0, 0);
...
void check_primitive(at *prim, void *info)
{
   if (CONSP(prim) && MODULEP(Car(prim))) {
      module_t *m = (module_t*)Mptr(Car(prim));
      if (check_executability)
         check_exec();
...
{
   if (m == root)
      return (char*)"Cannot unload root module";
   if (! (m->flags & MODULE_O))
      return (char*)"Cannot unload module of this type";
   if (m->flags & MODULE_STICKY)
      return (char*)"Cannot unload this module";
   /* Advertize */
   dynlink_hook(m, (char*)"unlink");
   /* Cleanup definitions */
   cleanup_module(m);
...
void module_unload(at *atmodule)
{
   ...
   if ((err = module_maybe_unload((module_t*)Mptr(atmodule))))
      RAISEF(err, atmodule);
   check_exec();
}
...
void dhclass_define(const char *name, dhclassdoc_t *kclass)
{
   ...
   class_t *cl = (class_t*)Mptr(classat);
   cl->priminame = module_priminame(symb); 
...
void dhmethod_define(dhclassdoc_t *kclass, const char *name, dhdoc_t *kname)
{
   ...
   class_t *cl = (class_t*)Mptr(kclass->lispdata.atclass);
   at *priminame = module_method_priminame(cl, symb);
   at *func = new_dh(priminame, kname);
   module_method_def(cl, symb, func);
}
...
extern "C" void pre_init_module(void)
{
   ...
   /* create root module */
   if (!root) {
      root = (module_t*)mm_alloc(mt_module);
...
extern "C" void init_module(char *progname)
{
   pre_init_module();
